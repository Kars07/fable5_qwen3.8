"""Modal Application for 4-Bit Quantized Distributed Data Parallel (DDP) Training on 4x NVIDIA A10G GPUs."""
...
# 1. Define Modal App
app = modal.App("fable5-qwen-4bit-ddp-training")
...
def run_4bit_ddp_training(
    ...
    """Execute 4-GPU 4-bit DDP training on 4x A10G GPUs."""
    os.chdir("/opt")
    print("=" * 70, flush=True)
    mode_desc = "SMOKE TEST (10 Steps)" if smoke_test else "FULL SFT RUN (3 Epochs)"
    print(f"[*] Starting {mode_desc} with 4-Bit NF4 Distributed Data Parallel on 4x A10G...", flush=True)
    ...
    # 1. Ensure model weights exist in volume
    ...
    if process.returncode != 0:
        raise RuntimeError(f"4-Bit DDP Training failed with exit code: {process.returncode}")

    if not smoke_test:
        volume_checkpoints.commit()
        print("[+] 4-Bit DDP SFT Training completed. Checkpoints committed to volume.", flush=True)
    else:
        print("[+] 4-Bit DDP Smoke test completed successfully!", flush=True)


@app.local_entrypoint()
def main(
    config: str = "configs/sft_train_4bit_ddp.yaml",
    smoke_test: bool = False,
    seq_len: int = 8192,
    r: int = 64,
    alpha: int = 128,
):
    """Local entrypoint for launching 4-bit DDP Modal training on 4x A10G."""
    print("=" * 70, flush=True)
    mode_str = "SMOKE TEST (10 Steps)" if smoke_test else "FULL SFT TRAINING (3 Epochs)"
    print(f"LAUNCHING 4-BIT DDP MODAL RUN: {mode_str}", flush=True)
    print(f"GPU Setup:       4x NVIDIA A10G (24GB each, 600 GB/s bandwidth)")
    print(f"Parallelism:     Distributed Data Parallel (DDP) - 4x Simultaneous Batches")
    print(f"Quantization:    4-Bit NF4 (Double Quant)")
    print(f"Context Length:  {seq_len} tokens")
    print(f"LoRA Config:     r={r}, alpha={alpha}")
    print("=" * 70, flush=True)
    run_4bit_ddp_training.remote(
        config_path=config,
