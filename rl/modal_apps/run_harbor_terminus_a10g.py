"""Regime 2: Terminal-Bench 2.1 (Terminus 2 + E2B MicroVMs) Evaluation on 4x NVIDIA A10G GPUs."""
...
app = modal.App("fable5-regime2-terminalbench-runner")

# Persistent Cloud Volumes
volume_hf = modal.Volume.from_name("hf-model-cache", create_if_missing=True)
volume_eval_outputs = modal.Volume.from_name("fable5-terminalbench-outputs", create_if_missing=True)

# Image definition with vLLM TP=4, Harbor 0.20, E2B microVMs
image = (
...
def run_regime2_eval(
    ...
    """Execute Regime 2 Level-3 End-to-End Evaluation on Terminal-Bench 2.1 via Terminus & E2B."""
    ...
    console.print(f"[bold cyan]🎯 REGIME 2: Terminal-Bench 2.1 / Terminus Evaluation on 4x NVIDIA A10G[/bold cyan]")
    console.print(f"[bold cyan]=======================================================================[/bold cyan]")
    ...
    if checkpoint_step != "base":
        if not (lora_dir / "adapter_model.safetensors").exists():
            console.print(f"[*] Downloading checkpoint {checkpoint_step} from Hugging Face Hub...")
            try:
                snapshot_download(
                ...
                console.print(f"[yellow][!] Checking base SFT repo for {checkpoint_step} ({e})...[/yellow]")
                try:
                    snapshot_download(
                    ...
                    console.print(f"[yellow][!] SFT sync note: {e2}[/yellow]")
    ...
    # 2. Launch Background vLLM OpenAI Server on 4x A10G (Tensor Parallelism = 4)
    out_dir = Path(f"/outputs/tb2_eval_{checkpoint_step}_{int(time.time())}")
    ...
    console.print(f"[*] Starting vLLM Server on 4x A10G (TP=4)...")
    ...
    try:
        ...
        # 3. Build & Execute Harbor Terminal-Bench Command with E2B MicroVMs
        ...
        console.print(f"[*] Executing Harbor Terminal-Bench 2.1 via Terminus 2 on E2B Sandboxes...")
        ...
        score_table = Table(title=f"Terminal-Bench 2.1 Official Automated Scorecard (Terminus 2 on E2B)", show_lines=True)
        ...
        score_table.add_row("Compute Topology", "4x NVIDIA A10G (96 GB VRAM)", "$4.40 / hr")
...
def main(
    ...
    """Local entrypoint for Terminal-Bench 2.1 evaluation."""
    run_regime2_eval.remote(
