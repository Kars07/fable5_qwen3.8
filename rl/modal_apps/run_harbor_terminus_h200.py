"""Regime 2 Level-3 End-to-End Terminal-Bench 2.1 Evaluation on 1x NVIDIA H200 SXM (141 GB VRAM, TP=1).

- Model: Qwen3.8-27B Base + Step 25 LoRA Adapter served via vLLM TP=1 on 1x NVIDIA H200 SXM (141 GB HBM3e)
- Memory Bandwidth: 4,800 GB/s (Fastest GPU in the world, zero inter-GPU communication latency)
- Context Length: 32,768 tokens (32K)
- Tool Calling: XML format (<command>...</command>)
- Autonomous Harness: Terminus 2 (Harbor 0.20)
...
app = modal.App("fable5-regime2-terminalbench-h200")
...
def run_regime2_eval_h200(
    ...
    """Execute Regime 2 Level-3 End-to-End Evaluation on Terminal-Bench 2.1 via Terminus & E2B on 1x NVIDIA H200 SXM."""
    ...
    console.print(f"[bold cyan]🎯 REGIME 2: Terminal-Bench 2.1 on 1x NVIDIA H200 SXM (141 GB, TP=1, 32K Context)[/bold cyan]")
    console.print(f"[bold cyan]=======================================================================[/bold cyan]")
    ...
    # 1. Sync LoRA Checkpoint (Only download the lightweight 2.4 GB adapter for step_25)
    ...
    if checkpoint_step != "base":
        ...
        # Standardize and sanitize LoRA adapter keys for vLLM compatibility
        if (lora_dir / "adapter_model.safetensors").exists():
            ...
            if needs_clean:
                console.print(f"[*] Sanitizing LoRA weight keys in {adapter_path} for canonical PEFT / vLLM compatibility...")
                clean_state_dict = {}
                ...
                volume_hf.commit()
                console.print(f"[+] Successfully sanitized and saved canonical PEFT adapter ({len(clean_state_dict)} tensors)")
    ...
    # 2. Launch Background vLLM OpenAI Server on 1x H200 SXM (Tensor Parallelism = 1, 131K Context)
    out_dir = Path(f"/outputs/tb2_eval_h200_{checkpoint_step}_{int(time.time())}")
    ...
    console.print(f"[*] Starting vLLM Server on 1x NVIDIA H200 SXM (141 GB VRAM, TP=1, 131K Context)...")
    ...
    try:
        ...
        console.print(f"[*] Executing Harbor Terminal-Bench 2.1 via Terminus 2 (XML Mode) on E2B Sandboxes...")
        ...
        score_table = Table(title=f"Terminal-Bench 2.1 Official Scorecard (1x H200 SXM, 141 GB VRAM, 32K Context)", show_lines=True)
        ...
        score_table.add_row("Compute Topology", "1x NVIDIA H200 SXM (141 GB VRAM, TP=1)", "$4.54 / hr")
        score_table.add_row("Context Length Window", "131,072 tokens (131K)", "Full Long-Context")
        score_table.add_row("Tool Calling Format", "XML (<command>...</command>)", "Native Code Support")
...
def main(
    ...
    """Local entrypoint for Terminal-Bench 2.1 evaluation on 1x H200 SXM."""
    run_regime2_eval_h200.remote(
